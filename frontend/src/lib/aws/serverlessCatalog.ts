/**
 * AWS Serverless Catalog - For ResourcePalette display
 * This provides the catalog entries that appear in the resource palette
 */

import { SERVERLESS_SERVICES, SERVERLESS_ICONS, ServerlessServiceDefinition } from './serverlessServicesData';
...
export function getServerlessServiceIcon(resourceType: string): string {
  return SERVERLESS_ICONS[resourceType] || SERVERLESS_ICONS['aws_lambda_function'];
}
