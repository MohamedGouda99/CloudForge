/**
 * AWS Security Catalog - For ResourcePalette display
 * This provides the catalog entries that appear in the resource palette
 */

import { SECURITY_SERVICES, SECURITY_ICONS, SecurityServiceDefinition } from './securityServicesData';
...
export const SECURITY_CATALOG: SecurityCatalogEntry[] = SECURITY_SERVICES.map(service => ({
  ...
  originalCategory: 'Security, Identity & Compliance',
...
export function getSecurityServiceIcon(resourceType: string): string {
  return SECURITY_ICONS[resourceType] || SECURITY_ICONS['aws_iam_role'];
}
