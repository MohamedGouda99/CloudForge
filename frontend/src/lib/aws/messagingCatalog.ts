/**
 * AWS Messaging Catalog - For ResourcePalette display
 * This provides the catalog entries that appear in the resource palette
 */

import { MESSAGING_SERVICES, MESSAGING_ICONS, MessagingServiceDefinition } from './messagingServicesData';
...
export const MESSAGING_CATALOG: MessagingCatalogEntry[] = MESSAGING_SERVICES.map(service => ({
  ...
  originalCategory: 'Application Integration',
...
export function getMessagingServiceIcon(resourceType: string): string {
  return MESSAGING_ICONS[resourceType] || MESSAGING_ICONS['aws_sqs_queue'];
}
